...
C5. Medición y analítica
 ...
 [SIGUIENTE] ANALYTICS-2. Completar y verificar dentro de GTM los tags de Meta Pixel y GA4, junto con los activadores de conversión confirmada. Estado parcial: CSP y `lead_submit_success` desplegados; GA4 base + `generate_lead` y Meta Pixel base + `Lead` están configurados en borrador con controles de consentimiento; píxel principal vinculado a la cuenta publicitaria. Tag Assistant aprobó el estado predeterminado denegado: ninguna etiqueta se activa. Pendiente antes de publicar: probar analítica solamente, marketing solamente, aceptar todo y un envío exitoso, y aprobar producción. Google Ads se separó como ANALYTICS-2B y no bloquea esta tarea. [código + configuración]
