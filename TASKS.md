...
C1. Formulario de Auditoría C.L.A.R.O. (reemplaza a A12, desglosado)
 [hecho] A12a. Server Action + integración HubSpot Forms API usando HUBSPOT_SERVICE_KEY: captura nombre, email, empresa, servicio de interés; envío del contacto a HubSpot. Sin anti-spam ni validación todavía — esta tarea es solo la conexión base. [código]
 [SIGUIENTE] A12b. Validación server-side con zod (no confiar solo en la validación del cliente) + checkbox de consentimiento explícito enlazado a /politica-de-privacidad (requiere LEGAL-1 hecho). [código]
