...
C1. Formulario de Auditoría C.L.A.R.O. (reemplaza a A12, desglosado)
 ...
 [SIGUIENTE — REQUIERE APROBACIÓN DEL USUARIO] FORM-NOTIFY-1. Enviar una notificación server-side a `promarketing2027@gmail.com` después de que Auditoría C.L.A.R.O. se procese correctamente en HubSpot, sin exponer credenciales en el cliente. Propuesta: Resend Free mediante API; mientras no exista dominio propio, registrar la cuenta con el mismo correo destinatario y usar el dominio de prueba solo para esa notificación interna. Requiere aprobación y API key antes de implementar. [código + configuración]
 B8-conectar. Conectar el newsletter al destino aprobado en FORM-SCOPE-1, con validación server-side, consentimiento y manejo correcto de errores; actualmente es solo una interfaz local. [código]
