...
C1. Formulario de Auditoría C.L.A.R.O. (reemplaza a A12, desglosado)
 ...
 FORM-NOTIFY-1. Enviar una notificación server-side a `promarketing2027@gmail.com` cuando un formulario se procese correctamente, sin exponer credenciales en el cliente ni considerar exitoso el envío antes de confirmar HubSpot. Antes de implementarlo, confirmar qué formularios incluye y elegir el proveedor de correo. [código + configuración]
...
en la primera impresión al compartir el link; accesibilidad y límites de plan al final porque no bloquean el lanzamiento)
 ...
 [SIGUIENTE] ACCESS-2a. Corregir en `components/education.tsx` el contraste de los textos inactivos y definir su comportamiento con movimiento reducido, sin romper la sincronización visual existente para usuarios sin esa preferencia. [código]
 ACCESS-2b. Corregir en la Política de Privacidad el contraste del aviso preliminar y eliminar el landmark `main` anidado. [código]
 ACCESS-2c. Corregir la jerarquía semántica de encabezados del Glosario sin introducir encabezados dentro de botones ni romper el Accordion compartido. [código]
 ACCESS-2d. Implementar la política global restante de movimiento reducido para Motion, Lenis y los diagramas animados. [código + configuración]
 ACCESS-VERIFY. Repetir Lighthouse y verificar navegación por teclado/foco visible después de corregir los hallazgos. BLOCKED hasta restablecer la comunicación de la extensión de Chrome. [auditoría]
