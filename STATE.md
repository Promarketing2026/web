...
2026-09-02 — TensionGrid Interactivo con Cards Expansibles y Mobile-First: Reimplementado `components/tension-grid.tsx` con comportamiento responsive diferenciado: expansión horizontal de columnas en desktop al hover (`2.25fr` vs `0.72fr`) con revelado de texto y micro-línea reactiva; acordeón táctil y scroll-tracking (`IntersectionObserver`) en mobile. Incorporados los íconos conceptuales alineados: `Split` (Identidad), `Unplug` (Tecnología) y `TrendingDown` (Precio y posicionamiento), conservando cabecera y cierre de causa raíz. Verificados: ESLint (0 errores), `tsc --noEmit` (0 errores) y Playwright E2E (`54/54 PASS`).
